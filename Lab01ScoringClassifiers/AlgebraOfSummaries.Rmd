...
# bad example: 
sub(BalancedAccuracy,TruePositives=0,FalsePositives=1,FalseNegatives=0,TrueNegatives=1)

# better treatment:
sub <- function(expr,TruePositives,FalsePositives,FalseNegatives,TrueNegatives) {
  eval(expr)
}
sub(parse(text=BalancedAccuracy),TruePositives=0,FalsePositives=1,FalseNegatives=0,TrueNegatives=1)
  ...
  for(TotFalse in 1:5) {
    for(TruePositives in 0:TotTrue) {
      for(TrueNegatives in 0:TotFalse) {
        ...
        fi = data.frame(F1=F1a,BalancedAccuracy=BAa)
...
ggplot(data=frm,aes(x=F1,y=BalancedAccuracy)) + geom_point()
```
