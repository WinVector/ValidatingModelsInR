...
# sovle
PrevalanceV = Var('Prevalance')
solns = sympy(paste('solve(',PrevalanceAlwaysYes,'-Prevalance,X)'))
solns = gsub('^\\[','',solns)
solns = gsub('\\]$','',solns)
solns = strsplit(solns,',')[[1]]
for(Xvalue in solns) {
  print(paste("x soln",Xvalue))
  PrevalanceAlwaysYesV = 
    sympy(paste('simplify((',PrevalanceAlwaysYes,').subs({X:',Xvalue,'}))'))
  print(paste("would entail Prevalance: ",PrevalanceAlwaysYesV))
  F1AlwaysYesV = 
    sympy(paste('simplify((',F1AlwaysYes,').subs({X:',Xvalue,'}))'))
  print(paste("would entail F1: ",F1AlwaysYesV))
}

# So F1 is 2*Prevalance/(1 + Prevalance) for classifiers that never say "no."
